...
export class YouTubeTranscriptProvider implements ITranscriptProvider {
  ...
  private async getPlayerResponse(expectedVideoId: string): Promise<any> {
    try {
      ...
      const mainWorldData = await getPlayerResponseFromMainWorld(expectedVideoId);
      if (mainWorldData && mainWorldData.videoId === expectedVideoId) return mainWorldData;
  ...
  async fetchTranscript(videoId: string, track: CaptionTrack, abortController?: AbortController): Promise<TranscriptResult> {
    const xmlText = await new Promise<string>((resolve, reject) => {
      ...
      chrome.runtime.sendMessage(
        { type: 'FETCH_CAPTION', requestId: Math.random().toString(), url: track.baseUrl },
        (response) => {
    ...
    const segments = parseXMLTranscript(xmlText, track.languageCode);
    ...
    const rawSegmentCount = xmlText.split('<text').length - 1;
